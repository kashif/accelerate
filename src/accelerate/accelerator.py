    ...
    **Available attributes:**
    ...
    def _prepare_torchtitan(self, *args):
        ...
        if scheduler is not None:
            ...
            # In multi-dimensional parallelism, scheduler might need to account for
            # different accumulation patterns across TP/PP dimensions
            if tp_size > 1 or pp_size > 1:
        ...
        for dataloader in dataloaders:
            logger.info("Preparing dataloader for TorchTitan sharded training")
            # Configure dataloader for multi-dimensional parallelism
            if dp_size > 1:
        ...
        for i, obj in enumerate(result):
            if isinstance(obj, torch.nn.Module) and model is not None:
                ...
                if dataloader_idx < len(prepared_dataloaders):
                    result[i] = prepared_dataloaders[dataloader_idx]
                    dataloader_idx += 1
                else:
